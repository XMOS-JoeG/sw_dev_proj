...
for i in range(1,8193):
  ...
  fb_div_list.append([i,0,0]) # This is when not using frac-n mode: integers only.
...
# So this doesn't constrain VCO freq because you have the output divider.
pll_out_max = 800.0 # 800MHz

def print_regs(op_div, fb_div, ref_div, fin_op_div):
  if args.app:
...
def print_solution(ppm_error, input_freq, out_freq, vco_freq, ref_div, fb_div, op_div, fin_op_div):
  ...
  print_regs(op_div, fb_div, ref_div, fin_op_div)

def print_solution_set(solutions):
  if args.raw:
    sol_str = ' Raw'
  else:
    sol_str = ' Filtered'
  ...
  for solution in solutions:
    print_solution(solution['ppm_error'], solution['input_freq'], solution['out_freq'], solution['vco_freq'], solution['ref_div'], solution['fb_div'], solution['op_div'], solution['fin_op_div'])
...
if args.raw:
  ...
  print_solution_set(final_filtered_solutions)
